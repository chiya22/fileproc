...
router.post("/insert", security.authorize(), (req, res, next) => {
  ...
  (async () => {
    try {
      const retObjUserCheck = await users.checkInterval(inObjUser.id, inObjUser.ymd_start, inObjUser.ymd_end, inObjUser.ymd_start, inObjUser.ymd_end);
...
router.post("/update", security.authorize(), (req, res, next) => {
  ...
  (async () => {
    try {
      const retObjUserCheck = await users.checkInterval(inObjUser.id, inObjUser.ymd_start, inObjUser.ymd_end, inObjUser.before_ymd_start, inObjUser.before_ymd_end);
      if (retObjUserCheck[0].length !== 0){
        req.flash("msg", "ユーザー【" + inObjUser.id + "】、適用開始日【" + inObjUser.ymd_start + "】適用終了日【" + inObjUser.ymd_end + "】は適用期間が重複しています");
        res.redirect(redirectUrl);
      } else {
